...
st.set_page_config(page_title="SEOtron 3000: Advanced Web Analysis Solution", layout="wide", page_icon="🚀")
    ...
    for attempt in range(retries):
        try:
            ...
            if attempt == retries - 1:
                return None  # Return None instead of string for consistent data type
...
def calculate_seo_score(result):
    score = 0
    # Load time scoring - handle None values
    if isinstance(result['load_time_ms'], (int, float)) and result['load_time_ms'] is not None:
        score += max(0, 20 - (result['load_time_ms'] / 100))
    
    score += 20 if result['mobile_friendly'] else 0
    
    # Readability scoring - handle potential None or invalid values
    try:
    ...
    # Image scoring
    if result['images'] and all(img['has_alt'] for img in result['images']):
        image_score = min(result['image_count'] / 5, 1) * 20
    else:
        image_score = 10
    score += image_score
...
def safe_mean(series):
    """Calculate mean while handling None values and mixed types"""
...
def analyze_url(url, target_keywords=None):
    ...
    result = {
        'url': url,
        'status': 'Success',
        'load_time_ms': None,  # Initialize as None instead of 0
        'meta_title': '',
        'meta_description': '',
        'meta_title_status': '',
    ...
    try:
        ...
        if result['load_time_ms'] is None:
            # Still continue with analysis even if load time fails
            pass
        ...
        # Handle readability calculation with error checking
        try:
            result['flesch_reading_ease'] = flesch_reading_ease(text_content) if text_content else 0
            result['flesch_kincaid_grade'] = flesch_kincaid_grade(text_content) if text_content else 0
            result['gunning_fog'] = gunning_fog(text_content) if text_content else 0
        except:
            result['flesch_reading_ease'] = 0
            result['flesch_kincaid_grade'] = 0
            result['gunning_fog'] = 0
        ...
        if urls or retry:
            ...
            if not df.empty and 'url' in df.columns:
                ...
                with tabs[0]:
                    ...
                    # Overview Table - Use safe_mean function for calculations
                    ...
                    summary_data = {
                        ...
                        "Value": [f"{safe_mean(df['load_time_ms']):.2f}", f"{safe_mean(df['word_count']):.0f}", 
                                  f"{safe_mean(df['internal_link_count']):.1f}", f"{safe_mean(df['external_link_count']):.1f}", 
                                  f"{safe_mean(df['image_count']):.1f}", f"{df['mobile_friendly'].sum()}", f"{len(df)}"]
                    }
                    ...
                    readability_data = {
                        ...
                        "Score": [f"{safe_mean(df['flesch_reading_ease']):.2f}", f"{safe_mean(df['flesch_kincaid_grade']):.2f}", 
                                  f"{safe_mean(df['gunning_fog']):.2f}", f"{safe_mean(df['seo_score']):.2f}"],
                        "Status": [
                            "Very Easy" if safe_mean(df['flesch_reading_ease']) >= 70 else "Moderate" if safe_mean(df['flesch_reading_ease']) >= 50 else "Difficult",
                            "Easy" if safe_mean(df['flesch_kincaid_grade']) <= 7 else "Average" if safe_mean(df['flesch_kincaid_grade']) <= 10 else "Advanced",
                            "Easy" if safe_mean(df['gunning_fog']) <= 8 else "Moderate" if safe_mean(df['gunning_fog']) <= 12 else "Complex",
                            "Excellent" if safe_mean(df['seo_score']) >= 80 else "Good" if safe_mean(df['seo_score']) >= 50 else "Needs Improvement"
                        ]
                    }
                    ...
                    try:
                        ...
                        if internal_links_data and external_links_data and headings_data and images_data:
                            full_report_data = {
                                'main_results': df,
                                'internal_links': pd.DataFrame(internal_links_data),
                                'external_links': pd.DataFrame(external_links_data),
                                'headings': pd.DataFrame(headings_data),
                                'images': pd.DataFrame(images_data)
                            }
                            # For simplicity, just download the main results
                            st.download_button("📥 Download Core Report", df.to_csv(index=False).encode('utf-8'), "seotron3000_report.csv", "text/csv", use_container_width=True)
                        else:
                            st.download_button("📥 Download Core Report", df.to_csv(index=False).encode('utf-8'), "seotron3000_report.csv", "text/csv", use_container_width=True)
                    except Exception as e:
                ...
                with tabs[1]:
                    ...
                    if available_columns:
                        ...
                        # Apply badge styling to the status columns that exist
                        status_columns = ['flesch_reading_ease_status', 'flesch_kincaid_grade_status', 'gunning_fog_status', 
                                        'https_status', 'indexability_status', 'meta_title_status', 'meta_description_status', 'header_status']
                        
                        for col in status_columns:
                            if col in main_df.columns:
                                main_df[col] = main_df[col].apply(apply_badge)
                        ...
                        st.download_button("📥 Download Core Metrics", df[available_columns].to_csv(index=False).encode('utf-8'), "core_metrics.csv", "text/csv", use_container_width=True)
                    else:
                        st.warning("No data available to display in main table.")
                ...
                with tabs[2]:
                    ...
                    if meta_data:
                        ...
                        st.info("No meta tag data available.")
                ...
                with tabs[3]:
                    ...
                    for result in results:
                        if result['status'] == "Success":
                            h1_text = next((h['text'] for h in result.get('headings', []) if h['level'] == 'H1'), "No H1")
                            header_data.append({
                                "URL": result['url'],
                    ...
                    if header_data:
                        ...
                        st.info("No header tag data available.")
                ...
                with tabs[4]:
                    ...
                    if internal_links_data:
                        ...
                        st.info("No internal links data available.")
                ...
                with tabs[5]:
                    ...
                    if external_links_data:
                        ...
                        st.info("No external links data available.")
                ...
                with tabs[6]:
                    ...
                    if headings_data:
                        ...
                        st.info("No headings data available.")
                ...
                with tabs[8]:
                    ...
                    if not df.empty and 'short_url' in df.columns:
                        try:
                            # Check if readability columns exist and have data
                            readability_cols = ['flesch_reading_ease', 'flesch_kincaid_grade', 'gunning_fog']
                            available_readability_cols = [col for col in readability_cols if col in df.columns and df[col].notna().any()]
                            
                            if available_readability_cols:
                                st.write("📖 Readability Scores Across URLs:")
                                fig = px.bar(
                                    df,
                                    x='short_url',
                                    y=available_readability_cols,
                            ...
                            if 'internal_link_count' in df.columns and 'external_link_count' in df.columns:
                                internal_sum = df['internal_link_count'].sum()
                                external_sum = df['external_link_count'].sum()
                                
                                if internal_sum > 0 or external_sum > 0:
                                    st.write("🔗 Link Distribution:")
                                    link_fig = px.pie(
                                        names=['Internal Links', 'External Links'],
                                        values=[internal_sum, external_sum],
                                        title="Link Types",
                            ...
                            st.info("Some visualization features may not be available due to insufficient data.")
                    else:
                        st.info("No data available for visualizations.")
            else:
                st.error("No successful results to display. Please check your URLs and try again.")
        else:
